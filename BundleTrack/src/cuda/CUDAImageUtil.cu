...
{
	template <class T>
	void copy(T *d_output, T *d_input, unsigned int width, unsigned int height)
	{
		cutilSafeCall(cudaMemcpy(d_output, d_input, sizeof(T) * width * height, cudaMemcpyDeviceToDevice));
	}

	template <>
	void copy<float>(float *d_output, float *d_input, unsigned int width, unsigned int height) { cutilSafeCall(cudaMemcpy(d_output, d_input, sizeof(float) * width * height, cudaMemcpyDeviceToDevice)); } // change by guan
	template <>
	void copy<uchar4>(uchar4 *d_output, uchar4 *d_input, unsigned int width, unsigned int height) { cutilSafeCall(cudaMemcpy(d_output, d_input, sizeof(uchar4) * width * height, cudaMemcpyDeviceToDevice)); } // change by guan
	...
	__host__ __device__ float convertToIntensity(const uchar4 &c)
	{
	...
	__global__ void computeNormalsSobel_Kernel(float4 *d_output, const float4 *d_input, unsigned int width, unsigned int height)
	{
		...
		if (x > 0 && x < width - 1 && y > 0 && y < height - 1)
		{
			float4 pos00 = d_input[(y - 1) * width + (x - 1)];
			if (pos00.x == MINF)
				return;
			float4 pos01 = d_input[(y - 0) * width + (x - 1)];
			if (pos01.x == MINF)
				return;
			float4 pos02 = d_input[(y + 1) * width + (x - 1)];
			if (pos02.x == MINF)
				return;

			float4 pos10 = d_input[(y - 1) * width + (x - 0)];
			if (pos10.x == MINF)
				return;
			// float4 pos11 = d_input[(y-0)*width + (x-0)]; if (pos11.x == MINF) return;
			float4 pos12 = d_input[(y + 1) * width + (x - 0)];
			if (pos12.x == MINF)
				return;

			float4 pos20 = d_input[(y - 1) * width + (x + 1)];
			if (pos20.x == MINF)
				return;
			float4 pos21 = d_input[(y - 0) * width + (x + 1)];
			if (pos21.x == MINF)
				return;
			float4 pos22 = d_input[(y + 1) * width + (x + 1)];
			if (pos22.x == MINF)
				return;
	...
	__global__ void computeCovisibilityKernel(const int H, const int W, const int stride, Eigen::Matrix4f *cur_in_kfcam, const float visible_angle_thres, const float4 *xyz_mapA, const float4 *normalA, int *n_visible, int *n_total_gpu)
	{
		...
		if (normalA_tmp.x == 0 && normalA_tmp.y == 0 && normalA_tmp.z == 0)
			return;
