...
// correspondence_idx -> image_Idx_i,j (i<j)
struct EntryJ
{
	...
	float3 normal_i;
	float3 normal_j;
	bool active = true; // For correspondence larger than some thres, we deactive it, and reactive later if within range again
...
public:
	...
	void reset()
	{
		...
		cudaMemset(d_globNumResiduals, 0, sizeof(int));
	...
	{
		cudaMemcpy(d_validImages, m_validImages.data(), sizeof(int) * getNumImages(), cudaMemcpyHostToDevice);
	}
	...
	{
		assert(numImages <= m_SIFTImagesGPU.size());
		if (numImages == m_SIFTImagesGPU.size())
			return;
	...
	void setGlobalCorrespondencesDEBUG(const std::vector<EntryJ> &correspondences)
	{
		...
		assert(correspondences.size() < MAX_MATCHES_PER_IMAGE_PAIR_FILTERED * (m_maxNumImages * (m_maxNumImages - 1)) / 2); // less than max #residuals
		cudaMemcpy(d_globMatches, correspondences.data(), sizeof(EntryJ) * correspondences.size(), cudaMemcpyHostToDevice);
		m_globNumResiduals = (unsigned int)correspondences.size();
		cudaMemcpy(d_globNumResiduals, &m_globNumResiduals, sizeof(unsigned int), cudaMemcpyHostToDevice);
	}
	...
	{
		assert(getNumImages() > 1);
		if (getCurrentFrame() + 1 == getNumImages())
			numMatches.resize(getNumImages() - 1);
		else
			numMatches.resize(getNumImages());
		cudaMemcpy(numMatches.data(), d_currNumMatchesPerImagePair, sizeof(unsigned int) * numMatches.size(), cudaMemcpyDeviceToHost);
	}
	void getNumFiltMatchesDEBUG(std::vector<unsigned int> &numMatches) const
	{
		...
		if (getCurrentFrame() + 1 == getNumImages())
			...
			numMatches.resize(getNumImages());
		cudaMemcpy(numMatches.data(), d_currNumFilteredMatchesPerImagePair, sizeof(unsigned int) * numMatches.size(), cudaMemcpyDeviceToHost);
	}
	void getSIFTKeyPointsDEBUG(std::vector<SIFTKeyPoint> &keys) const
	{
		keys.resize(m_numKeyPoints);
		cudaMemcpy(keys.data(), d_keyPoints, sizeof(SIFTKeyPoint) * keys.size(), cudaMemcpyDeviceToHost);
	}
	void getSIFTKeyPointDescsDEBUG(std::vector<SIFTKeyPointDesc> &descs) const
	{
		descs.resize(m_numKeyPoints);
		cudaMemcpy(descs.data(), d_keyPointDescs, sizeof(SIFTKeyPointDesc) * descs.size(), cudaMemcpyDeviceToHost);
	}
	void getRawKeyPointIndicesAndMatchDistancesDEBUG(unsigned int imagePairIndex, std::vector<uint2> &keyPointIndices, std::vector<float> &matchDistances) const
	{
		...
		cudaMemcpy(&numMatches, d_currNumMatchesPerImagePair + imagePairIndex, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		if (numMatches > MAX_MATCHES_PER_IMAGE_PAIR_RAW)
			numMatches = MAX_MATCHES_PER_IMAGE_PAIR_RAW;
		keyPointIndices.resize(numMatches);
		matchDistances.resize(numMatches);
		if (numMatches > 0)
		{
			cudaMemcpy(keyPointIndices.data(), d_currMatchKeyPointIndices + imagePairIndex * MAX_MATCHES_PER_IMAGE_PAIR_RAW, sizeof(uint2) * numMatches, cudaMemcpyDeviceToHost);
			cudaMemcpy(matchDistances.data(), d_currMatchDistances + imagePairIndex * MAX_MATCHES_PER_IMAGE_PAIR_RAW, sizeof(float) * numMatches, cudaMemcpyDeviceToHost);
		}
	}
	...
	{
		unsigned int numMatches;
		cudaMemcpy(&numMatches, d_currNumFilteredMatchesPerImagePair + imagePairIndex, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		assert(numMatches <= MAX_MATCHES_PER_IMAGE_PAIR_FILTERED);
		keyPointIndices.resize(numMatches);
		matchDistances.resize(numMatches);
		if (numMatches > 0)
		{
			cudaMemcpy(keyPointIndices.data(), d_currFilteredMatchKeyPointIndices + imagePairIndex * MAX_MATCHES_PER_IMAGE_PAIR_FILTERED, sizeof(uint2) * numMatches, cudaMemcpyDeviceToHost);
			cudaMemcpy(matchDistances.data(), d_currFilteredMatchKeyPointIndices + imagePairIndex * MAX_MATCHES_PER_IMAGE_PAIR_FILTERED, sizeof(float) * numMatches, cudaMemcpyDeviceToHost);
		}
	}
	void getCurrMatchKeyPointIndicesDEBUG(std::vector<uint2> &keyPointIndices, std::vector<unsigned int> &numMatches, bool filtered) const
	{
		...
		if (filtered)
		{
			keyPointIndices.resize(numMatches.size() * MAX_MATCHES_PER_IMAGE_PAIR_FILTERED);
			cudaMemcpy(keyPointIndices.data(), d_currFilteredMatchKeyPointIndices, sizeof(uint2) * keyPointIndices.size(), cudaMemcpyDeviceToHost);
		}
		else
		{
			keyPointIndices.resize(numMatches.size() * MAX_MATCHES_PER_IMAGE_PAIR_RAW);
			cudaMemcpy(keyPointIndices.data(), d_currMatchKeyPointIndices, sizeof(uint2) * keyPointIndices.size(), cudaMemcpyDeviceToHost);
		}
	}
