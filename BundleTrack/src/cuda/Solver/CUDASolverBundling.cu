...
{
	if (m_timer)
		m_timer->startEvent(__FUNCTION__);
		...
		if (maxResidual > GUIDED_SEARCH_MAX_RES_THRESH)
		{
			...
			if (n > 1)
			{
				...
				for (unsigned int i = 0; i < n; i++)
				{
					...
					{																													// introduced by latest image
						...
						if (prevMaxRes > 1.5f * m_solverExtra.h_maxResidual[i])
						{
							...
							if (it == residualMap.end())
								residualMap[imageIndices] = m_solverExtra.h_maxResidual[i];
							else
								it->second = std::max(m_solverExtra.h_maxResidual[i], it->second);
						}
					}
					else if (h_corr.imgIdx_j == revalidateIdx && std::abs((int)h_corr.imgIdx_i - (int)h_corr.imgIdx_j) > 10)
					{ // introduced by latest revalidate
						auto it = residualMap.find(imageIndices);
						if (it == residualMap.end())
							residualMap[imageIndices] = m_solverExtra.h_maxResidual[i];
						else
					...
					if (it == allCollectedResidualMap.end())
						allCollectedResidualMap[imageIndices] = m_solverExtra.h_maxResidual[i];
					else
						it->second = std::max(m_solverExtra.h_maxResidual[i], it->second);
				}
				...
				{ // debug print
					...
					for (const auto &r : residualMap)
						m_maxResImPairs.push_back(r.first);
...
bool CUDASolverBundling::getMaxResidual(unsigned int curFrame, EntryJ *d_correspondences, Eigen::Vector<uint, 2> &imageIndices, float &maxRes)
{
	...
	if (!(imageIndices(0) == 0 && imageIndices(1) < 10) && m_solverExtra.h_maxResidual[0] > curThresh)
		remove = true; // don't remove the first frame
...
bool CUDASolverBundling::useVerification(EntryJ *d_correspondences, unsigned int numberOfCorrespondences)
{
	...
	if ((float)numHighResiduals / solverInput.numberOfCorrespondences >= parameters.verifyOptPercentThresh)
		return true;
	return false;
}
