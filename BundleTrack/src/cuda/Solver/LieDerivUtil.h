...
/////////////////////////////////////////////////////////////////////////
// deriv for Ti: (A * e^e * D)^{-1} * pj  <---> (Tj^(-1)*Ti)^(-1)=Ti^(-1)*Tj; A = Tj^{-1}; D = Ti, where Tj and Ti are pose from cam to world
/////////////////////////////////////////////////////////////////////////
...
__inline__ __device__ matNxM<3, 6> evalLie_derivJ(const float4x4 &A, const float4x4 &D, const float3 &p)
{
	...
	jac(2, 2) = 1.0f;
	jac(0, 3) = 0.0f;
	jac(0, 4) = dot(p, dr3) + dtz;
	jac(0, 5) = -(dot(p, dr2) + dty);
	jac(1, 3) = -(dot(p, dr3) + dtz);
	jac(1, 4) = 0.0f;
	jac(1, 5) = dot(p, dr1) + dtx;
	jac(2, 3) = dot(p, dr2) + dty;
	jac(2, 4) = -(dot(p, dr1) + dtx);
	jac(2, 5) = 0.0f;
...
__inline__ __device__ void computeLieUpdate(const float3 &updateW, const float3 &updateT, const float3 &curW, const float3 &curT, float3 &newW, float3 &newT)
{
