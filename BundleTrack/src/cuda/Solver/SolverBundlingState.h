...
struct SolverInput
{
	...
	unsigned int numberOfCorrespondences; // total number of sparse correspondence
...
struct SolverState
{
	...
	float3 *d_rRot;	  // Residuum // J^T * r
	float3 *d_rTrans; // Residuum // J^T * r

	float3 *d_zRot;	  // Preconditioned residuum
	float3 *d_zTrans; // Preconditioned residuum

	float3 *d_pRot;	  // Decent direction for rotation (including preconditioner)
	...
	float *d_scanAlpha; // Tmp memory for alpha and beta scan
	...
	float4x4 *d_xTransforms;		// cam in world
	float4x4 *d_xTransformInverses; // world in cam
