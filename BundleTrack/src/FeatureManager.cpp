...
void SiftManager::rejectFeatures(std::shared_ptr<Frame> frame)
{
  ...
  for (int i = 0; i < kpts_tmp.size(); i++)
  {
    ...
    if (!Utils::isPixelInsideImage(H, W, u, v))
      continue;
    if (frame->_depth.at<float>(v, u) < 0.1)
      continue;
...
void SiftManager::pruneMatches(std::shared_ptr<Frame> frameA, std::shared_ptr<Frame> frameB, const std::vector<std::vector<cv::DMatch>> &knn_matchesAB, std::vector<cv::DMatch> &matches_AB)
{
  ...
  for (int i = 0; i < knn_matchesAB.size(); i++)
  {
    for (int k = 0; k < knn_matchesAB[i].size(); k++)
    {
      ...
      if (dist > dist_thres || n1.normalized().dot(n2.normalized()) < normal_thres)
        continue;
...
{
  if (_covisible_mappoints.find({frameA, frameB}) != _covisible_mappoints.end())
    return _covisible_mappoints[{frameA, frameB}];
  if (_covisible_mappoints.find({frameB, frameA}) != _covisible_mappoints.end())
    return _covisible_mappoints[{frameB, frameA}];
...
void SiftManager::findCorresbyGroundtruth(std::shared_ptr<Frame> frameA, std::shared_ptr<Frame> frameB)
{
  if (_matches.find({frameA, frameB}) != _matches.end())
    return;
  if (_matches.find({frameB, frameA}) != _matches.end())
    return;
...
void SiftManager::runRansacBetween(std::shared_ptr<Frame> frameA, std::shared_ptr<Frame> frameB)
{
  ...
  for (int i = 0; i < samples_to_try.size(); i++)
  {
    ...
    if (offset == Eigen::Matrix4f::Identity())
      continue;
    ...
    for (int i = 0; i < matches.size(); i++)
    {
      ...
      if (n1.normalized().dot(n2.normalized()) < cos_normal_angle)
        continue;
