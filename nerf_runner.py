...
def compute_near_far_and_filter_rays(cam_in_world, rays, cfg):
    ...
    near = np.abs(near)
    far = np.abs(far)
    good_rays = np.concatenate(
        (good_rays, near.reshape(-1, 1), far.reshape(-1, 1)), axis=-1
    )  # (N,8+2)
...
def sample_rays_uniform(N_samples, near, far, lindisp=False, perturb=True):
    ...
    """
    N_ray = near.shape[0]
    t_vals = torch.linspace(0.0, 1.0, steps=N_samples, device=near.device).reshape(
        1, -1
    )
    if not lindisp:
...
class NerfRunner:
    def __init__(
        ...
        self.build_octree_pts = np.asarray(
            build_octree_pcd.points
        ).copy()  # Make it pickable
        ...
        if down_scale_ratio != 1:
            ...
            if normal_maps is not None:
                self.normal_maps = normal_maps[
                    :, ::down_scale_ratio, ::down_scale_ratio
                ]
            if occ_masks is not None:
                self.occ_masks = occ_masks[:, ::down_scale_ratio, ::down_scale_ratio]
            self.H, self.W = self.images.shape[1:3]
            self.cfg["dilate_mask_size"] = int(
                self.cfg["dilate_mask_size"] // down_scale_ratio
            )
    ...
    def create_nerf(self, device=torch.device("cuda")):
        ...
        if self.cfg["use_viewdirs"]:
            embeddirs_fn, input_ch_views = get_embedder(
                self.cfg["multires_views"],
                self.cfg,
                i=self.cfg["i_embed_views"],
                octree_m=self.octree_m,
            )
        ...
        model = NeRFSmall(
            ...
            hidden_dim_color=64,
            input_ch=input_ch,
            input_ch_views=input_ch_views + self.cfg["frame_features"],
        ).to(device)
        ...
        if self.cfg["frame_features"] > 0:
            feature_array = FeatureArray(
                num_training_frames, self.cfg["frame_features"]
            ).to(device)
    ...
    def make_frame_rays(self, frame_id):
        ...
        rays = get_camera_rays_np(
            self.H, self.W, self.K
        )  # [self.H, self.W, 3]  We create rays frame-by-frame to save memory
        ...
        rays = np.concatenate(
            [rays, frame_id * np.ones(self.depths[frame_id].shape)], -1
        )  # [H, W, 12]
        ray_types = np.zeros(
            (self.H, self.W, 1)
        )  # 0 is good; 1 is invalid depth (uncertain)
        invalid_depth = (
            (self.depths[frame_id, ..., 0] < self.cfg["near"] * self.cfg["sc_factor"])
        ...
        cur_rays = compute_near_far_and_filter_rays(
            self.poses[frame_id], cur_rays, self.cfg
        )
        if self.normal_maps is not None:
        ...
        if self.cfg["use_octree"]:
            rays_o_world = (
                self.poses[frame_id] @ to_homo(np.zeros((len(cur_rays), 3))).T
            ).T[:, :3]
            rays_o_world = torch.from_numpy(rays_o_world).cuda().float()
            rays_unit_d_cam = cur_rays[:, :3] / np.linalg.norm(
                cur_rays[:, :3], axis=-1
            ).reshape(-1, 1)
    ...
    def compute_rays_z_in_out(self):
        ...
        near, far, _, depths_in_out = self.octree_m.ray_trace(
            rays_o_w, viewdirs_w, level=level, debug=0
        )
        N_intersect = depths_in_out.shape[1]

        ########### Convert the time to Z
        z_in_out = (
            depths_in_out.cuda() * torch.abs(viewdirs[..., 2].view(N_rays, 1, 1))
        ).cuda()
        ...
        valid = (depths >= self.cfg["near"] * self.cfg["sc_factor"]) & (
            depths <= self.cfg["far"] * self.cfg["sc_factor"]
        ).expand(-1, N_intersect)
        valid = valid & (z_in_out > 0).all(dim=-1)  # (N_ray, N_intersect)
        z_in_out[valid] = torch.clip(
            z_in_out[valid],
            min=torch.zeros_like(z_in_out[valid]),
            max=torch.ones_like(z_in_out[valid])
            * (depths.reshape(-1, 1, 1).expand(-1, N_intersect, 2)[valid] + trunc),
        )
    ...
    def add_new_frames(
        ...
        if not reuse_weights:
            ...
            if self.cfg["frame_features"] > 0:
                feature_array = FeatureArray(
                    len(self.images), self.cfg["frame_features"]
                ).to(self.rays.device)
                if reuse_weights:
                    for i in range(prev_n_image):
                        feature_array.data.data[i] = (
                            self.models["feature_array"].data.data[i].detach().clone()
                        )
        ...
        if not self.cfg["no_batching"]:
            ...
            if self.cfg["denoise_depth_use_octree_cloud"]:
                logging.info("denoise cloud")
                mask = (rays[:, self.ray_mask_slice] > 0) & (
                    rays[:, self.ray_depth_slice]
                    <= self.cfg["far"] * self.cfg["sc_factor"]
                )
                rays_dir = rays[mask][:, self.ray_dir_slice]
                rays_depth = rays[mask][:, self.ray_depth_slice]
                pts3d = rays_dir * rays_depth.reshape(-1, 1)
    ...
    def build_octree(self):
        ...
        pts = (
            torch.tensor(self.build_octree_pts).cuda().float()
        )  # Must be within [-1,1]
        octree_smallest_voxel_size = (
            self.cfg["octree_smallest_voxel_size"] * self.cfg["sc_factor"]
        )
        ...
        dilate_radius = int(
            np.ceil(
                self.cfg["octree_dilate_size"] / self.cfg["octree_smallest_voxel_size"]
            )
        )
    ...
    def create_optimizer(self):
        ...
        if self.models["pose_array"] is not None:
            param_groups.append(
                {
                    "name": "pose_array",
                    "params": self.models["pose_array"].parameters(),
                    "lr": self.cfg["lrate_pose"],
                }
            )

        self.optimizer = torch.optim.Adam(
            param_groups, betas=(0.9, 0.999), weight_decay=0, eps=1e-15
        )
        ...
        for k in self.models.keys():
            ...
            if self.models[k] is not None:
                if k in ["pose_array", "feature_array"]:
                    self.models[k].data.data = torch.cat(
                        [
                            other.models[k].data[:n_frames_other].detach(),
                            self.models[k].data[n_frames_other:].detach(),
                        ],
                        dim=0,
                    )
    ...
    def schedule_lr(self):
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            new_lrate = init_lr * (
                self.cfg["decay_rate"] ** (float(self.global_step) / self.N_iters)
            )
    ...
    def render_images(self, img_i, cur_rays=None):
        ...
        ray_ids = torch.arange(len(self.rays), device=cur_rays.device)[
            frame_ids == img_i
        ].long()
    ...
    def gradient_clip(self):
        ...
        torch.nn.utils.clip_grad_norm_(
            params,
            max_norm=self.cfg["gradient_max_norm"],
            norm_type="inf",
            error_if_nonfinite=error_if_nonfinite,
        )
        if self.models["pose_array"] is not None:
            torch.nn.utils.clip_grad_norm_(
                self.models["pose_array"].data,
                max_norm=self.cfg["gradient_pose_max_norm"],
                norm_type="inf",
                error_if_nonfinite=error_if_nonfinite,
            )
    ...
    def get_truncation(self):
        ...
        if self.cfg["trunc_decay_type"] == "linear":
            truncation = (
                self.cfg["trunc_start"]
                - (self.cfg["trunc_start"] - self.cfg["trunc"])
                * float(self.global_step)
                / self.cfg["n_step"]
            )
        elif self.cfg["trunc_decay_type"] == "exp":
            lamb = np.log(self.cfg["trunc"] / self.cfg["trunc_start"]) / (
                self.cfg["n_step"] / 4
            )
    ...
    def train_loop(self, batch):
        ...
        rgb, extras = self.render(
            rays=batch,
            ray_ids=self.data_loader.batch_ray_ids,
            frame_ids=frame_ids,
            depth=target_d,
            lindisp=False,
            perturb=True,
            raw_noise_std=self.cfg["raw_noise_std"],
            near=batch[:, self.ray_near_slice],
            far=batch[:, self.ray_far_slice],
            get_normals=False,
        )
        ...
        sample_weights = (
            ray_weights.view(N_rays, 1).expand(-1, N_samples) * valid_samples
        )
        ...
        if "rgb0" in extras:
            img_loss0 = (
                ((extras["rgb0"] - target_s) ** 2 * ray_weights.view(-1, 1))
            ).mean()
        ...
        if self.cfg["depth_weight"] > 0:
            ...
            depth_loss = (
                (z_min * weights - depth.view(-1, 1) * weights) ** 2
            ).mean() * self.cfg["depth_weight"]
            loss = loss + depth_loss

        truncation = self.get_truncation()
        sample_weights[ray_type == 1] = 0
        fs_loss, sdf_loss, front_mask, sdf_mask = get_sdf_loss(
            z_vals,
            target_d.reshape(-1, 1).expand(-1, N_samples),
            sdf,
            truncation,
            self.cfg,
            return_mask=True,
        ...
        if self.cfg["eikonal_weight"] > 0:
            ...
            eikonal_loss = (
                (torch.norm(nerf_normals[sdf < 1], dim=-1) - 1) ** 2
            ).mean() * self.cfg["eikonal_weight"]
            loss += eikonal_loss
        ...
        if self.models["feature_array"] is not None:
            reg_features = (
                self.cfg["feature_reg_weight"]
                * (self.models["feature_array"].data ** 2).mean()
            )
            loss += reg_features
        ...
        if self.global_step % self.cfg["i_weights"] == 0 and self.global_step > 0:
            self.save_weights(
                out_file=os.path.join(self.cfg["save_dir"], f"model_latest.pth"),
                models=self.models,
            )

        if self.global_step % self.cfg["i_img"] == 0 and self.global_step > 0:
            ids = (
                torch.unique(self.rays[:, self.ray_frame_id_slice])
                .data.cpu()
                .numpy()
                .astype(int)
                .tolist()
            ...
            for frame_idx in ids:
                rgb, depth, ray_mask, gt_rgb, gt_depth, _ = self.render_images(
                    frame_idx
                )
                ...
                gt_depth = np.clip(
                    gt_depth, self.cfg["near"] * self.cfg["sc_factor"], far
                )
                depth_vis = np.concatenate(
                    (to8b(depth / far), to8b(gt_depth / far)), axis=1
                )
        ...
        if self.global_step % self.cfg["i_mesh"] == 0 and self.global_step > 0:
            with torch.no_grad():
                model = (
                    self.models["model_fine"]
                    if self.models["model_fine"] is not None
                    else self.models["model"]
                )
                mesh = self.extract_mesh(
                    isolevel=0, voxel_size=self.cfg["mesh_resolution"]
                )
        ...
        if self.global_step % self.cfg["i_pose"] == 0 and self.global_step > 0:
            ...
            dir = os.path.join(
                self.cfg["save_dir"], f"step_{self.global_step:07d}_optimized_poses.txt"
            )
    ...
    def make_key_ray_ids(self):
        ...
        for k in matches_table.keys():
            ...
            matches_table[k] = matches_table[k] / np.tile(
                self.down_scale.reshape(1, 2), (1, 2)
            )
            ...
            def kpts_to_ray_ids(kpts, id, dilate=True):
                cur_frame_mask = (
                    (self.rays[:, self.ray_frame_id_slice] == id).data.cpu().numpy()
                )
                ...
                if dilate:
                    steps = np.array(
                        list(itertools.product(np.arange(-5, 6), repeat=2))
                    ).reshape(-1, 2)
            ...
            def rays_to_pts_world(rays):
                ...
                frame_ids = (
                    rays[:, self.ray_frame_id_slice].data.cpu().numpy().astype(int)
                )
            ...
            def rays_to_vox_ids(rays):
                ...
                (
                    ...
                    rays_pid,
                    ray_depths_in_out,
                ) = self.octree_m.ray_trace(rays_o_w, rays_dir_w, level=level)
                return rays_pid
    ...
    def train_BA(self):
        for self.global_step in range(200):
            ...
            valid = (
                rayA[:, self.ray_depth_slice] <= self.cfg["far"] * self.cfg["sc_factor"]
            ) & (
                rayB[:, self.ray_depth_slice] <= self.cfg["far"] * self.cfg["sc_factor"]
            )
            rayA = rayA[valid]
            rayB = rayB[valid]
            ...
            if self.global_step % self.cfg["i_pose"] == 0 and self.global_step > 0:
                if self.models["pose_array"] is not None:
                    ...
                    optimized_poses = self.poses
                dir = os.path.join(
                    self.cfg["save_dir"], f"step_{self.global_step}_optimized_poses.txt"
                )
                np.savetxt(dir, optimized_poses.reshape(-1, 4))
                if self._run is not None:
                    self._run.add_artifact(dir)
    ...
    @torch.no_grad()
    def sample_rays_uniform_occupied_voxels(
        self,
        ray_ids,
        rays_d,
        depths_in_out,
        ...
        ########### Convert the time to Z
        z_in_out = (
            depths_in_out.cuda() * torch.abs(dirs[..., 2]).reshape(N_rays, 1, 1).cuda()
        )
        ...
        z_vals_continous = sample_rays_uniform(
            N_samples,
            torch.zeros((N_rays, 1), device=z_in_out.device).reshape(-1, 1),
            depths_lens.sum(dim=-1).reshape(-1, 1),
            lindisp=lindisp,
            perturb=perturb,
        )  # (N_ray,N_sample)
        ...
        z_vals = torch.zeros(
            (N_rays, N_samples), dtype=torch.float, device=rays_d.device
        )
        z_vals = common.sampleRaysUniformOccupiedVoxels(
            z_in_out.contiguous(), z_vals_continous.contiguous(), z_vals
        )
    ...
    def render_rays(
        ...
        z_vals, _ = self.sample_rays_uniform_occupied_voxels(
            ray_ids=ray_ids,
            rays_d=viewdirs,
        ...
        if (
            self.cfg["N_samples_around_depth"] > 0 and depth is not None
        ):  #!NOTE only fine when depths are all valid
            ...
            z_vals_around_depth = torch.zeros(
                (N_rays, self.cfg["N_samples_around_depth"]), device=ray_batch.device
            ).float()
            # if torch.sum(inside_mask)>0:
            z_vals_around_depth[valid_depth_mask] = sample_rays_uniform(
                self.cfg["N_samples_around_depth"],
                near_depth.reshape(-1, 1),
                far_depth.reshape(-1, 1),
                lindisp=lindisp,
                perturb=perturb,
            )
            ...
            if invalid_depth_mask.any() and self.cfg["use_octree"]:
                ...
                z_vals_invalid, _ = self.sample_rays_uniform_occupied_voxels(
                    ray_ids=ray_ids[invalid_depth_mask.to(ray_ids.device)],
                    rays_d=viewdirs[invalid_depth_mask.to(ray_ids.device)],
                    depths_in_out=depths_in_out[invalid_depth_mask.to(ray_ids.device)],
                    lindisp=lindisp,
                    perturb=perturb,
                    depths=None,
                    N_samples=self.cfg["N_samples_around_depth"],
                )
                z_vals_around_depth[
                    invalid_depth_mask.to(z_vals_around_depth.device)
                ] = z_vals_invalid
            else:
                z_vals_around_depth[
                    invalid_depth_mask.to(z_vals_around_depth.device)
                    ...
                    self.cfg["N_samples_around_depth"],
                    near[invalid_depth_mask.to(near.device)].reshape(-1, 1),
                    far[invalid_depth_mask.to(far.device)].reshape(-1, 1),
                    lindisp=lindisp,
                    perturb=perturb,
                )
            ...
            valid_samples = torch.ones(
                z_vals.shape, dtype=torch.bool, device=ray_batch.device
            )  # During pose update if ray out of box, it becomes invalid
        ...
        deformation = None
        raw, normals, valid_samples = self.run_network(
            pts,
            viewdirs,
            frame_ids,
            tf=tf,
        ...
        rgb_map, weights = self.raw2outputs(
            raw,
            z_vals,
            rays_d,
        ...
        if self.cfg["N_importance"] > 0:
            ...
            for iter in range(self.cfg["N_importance_iter"]):
                ...
                valid_samples_importance = torch.ones(
                    z_samples.shape, dtype=torch.bool
                ).to(z_vals.device)
                valid_samples_importance[
                    torch.all(valid_samples == 0, dim=-1).reshape(-1)
                ] = 0

                if (
                    self.models["model_fine"] is not None
                    and self.models["model_fine"] != self.models["model"]
                ):
                    z_vals, _ = torch.sort(torch.cat([z_vals, z_samples], -1), -1)
                    pts = (
                        rays_o[..., None, :]
                        + rays_d[..., None, :] * z_vals[..., :, None]
                    )  # [N_rays, N_samples + self.cfg['N_importance'], 3]
                    raw, normals, valid_samples = self.run_network(
                        pts,
                        viewdirs,
                        frame_ids,
                    ...
                    pts = (
                        rays_o[..., None, :]
                        + rays_d[..., None, :] * z_samples[..., :, None]
                    )  # [N_rays, N_samples + self.cfg['N_importance'], 3]
                    raw_fine, valid_samples_importance = self.run_network(
                        pts,
                        viewdirs,
                        frame_ids,
                        tf=tf,
                        valid_samples=valid_samples_importance,
                        get_normals=False,
                    )
                    ...
                    raw = torch.gather(
                        torch.cat([raw, raw_fine], dim=1),
                        dim=1,
                        index=indices[..., None].expand(-1, -1, raw.shape[-1]),
                    )
                    valid_samples = torch.cat(
                        (valid_samples, valid_samples_importance), dim=-1
                    )
    ...
    def raw2outputs(
        self, raw, z_vals, rays_d, raw_noise_std=0, valid_samples=None, depth=None
    ):
        ...
        def sdf2weights(sdf):
            ...
            weights = torch.sigmoid(
                sdf_from_depth * self.cfg["sdf_lambda"]
            ) * torch.sigmoid(
                -sdf_from_depth * self.cfg["sdf_lambda"]
            )  # This not work well

            invalid = (depth > self.cfg["far"] * self.cfg["sc_factor"]).reshape(-1)
            mask = (z_vals - depth <= truncation * self.cfg["neg_trunc_ratio"]) & (
                z_vals - depth >= -truncation
            )
    ...
    def run_network(
        ...
        if valid_samples is None:
            valid_samples = torch.ones(
                (N_ray, N_sample), dtype=torch.bool, device=inputs.device
            )
        ...
        valid_samples = (
            valid_samples.bool()
            & (torch.abs(inputs_flat) <= 1).all(dim=-1).view(N_ray, N_sample).bool()
        )

        embedded = torch.zeros(
            (inputs_flat.shape[0], self.models["embed_fn"].out_dim),
            device=inputs_flat.device,
        )
        if valid_samples is None:
            valid_samples = torch.ones(
                (N_ray, N_sample), dtype=torch.bool, device=inputs_flat.device
            )
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg["amp"]):
            if self.cfg["i_embed"] in [3]:
                embedded[valid_samples.reshape(-1)], valid_samples_embed = self.models[
                    "embed_fn"
                ](inputs_flat[valid_samples.reshape(-1)])
        ...
        if self.models["feature_array"] is not None:
            if latent_code is None:
                ...
                frame_features = (
                    frame_features[:, None].expand(-1, N_sample, -1).reshape(-1, D)
                )
            else:
                D = latent_code.shape[-1]
                frame_features = (
                    latent_code[:, None]
                    .expand(N_ray, N_sample, latent_code.shape[-1])
                    .reshape(-1, D)
                )
        ...
        if self.models["embeddirs_fn"] is not None:
            ...
            embedded_dirs_flat = (
                embedded_dirs[:, None].expand(-1, N_sample, *tmp).reshape(-1, *tmp)
            )
    ...
    def run_network_density(self, inputs, get_normals=False):
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg["amp"]):
            if self.cfg["i_embed"] in [3]:
                ...
                bad_ids = prev_valid_ids[valid_samples_embed == 0]
                new_valid_ids = torch.ones(
                    (len(inputs_flat)), device=inputs.device
                ).bool()
                new_valid_ids[bad_ids] = 0
                valid_samples = valid_samples & new_valid_ids
            else:
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg["amp"]):
            chunk = self.cfg["netchunk"]
            for i in range(0, embedded.shape[0], chunk):
                alpha = self.models["model"].forward_sdf(
                    embedded[i : i + chunk]
                )  # (N,1)
    ...
    def extract_mesh(
        ...
        vertices[:, :3] = voxel_size_ndc.reshape(1, 3) * vertices[
            :, :3
        ] + offset.reshape(1, 3)
    ...
    def mesh_vertex_color_from_network(self, mesh):
        ...
        with torch.no_grad():
            ...
            for b in range(0, len(inputs), bs):
                outputs_local, normals, valid_samples = self.run_network(
                    inputs=inputs[b : b + bs],
                    viewdirs=viewdirs[b : b + bs],
                    frame_ids=frame_ids[b : b + bs],
                    tf=tf[b : b + bs],
                )
        ...
        visual = trimesh.visual.ColorVisuals(
            mesh=mesh, face_colors=None, vertex_colors=(colors * 255).astype(np.uint8)
        )
        mesh.visual = visual
        return mesh
    ...
    def mesh_vertex_color_from_train_images(self, mesh):
        ...
        for i in range(len(self.images)):
            ...
            valid = (depth >= 0.1 * self.cfg["sc_factor"]) & (
                self.masks[i].reshape(self.H, self.W).astype(bool)
            )
        ...
        visual = trimesh.visual.ColorVisuals(
            mesh=mesh, face_colors=None, vertex_colors=vertex_colors.astype(np.uint8)
        )
        mesh.visual = visual
        return mesh

    def mesh_texture_from_train_images(
        self, mesh, rgbs_raw, train_texture=False, tex_res=1024
    ):
        """
        ...
        for i in range(len(rgbs_raw)):
            ...
            valid = (
                render_depth.reshape(self.H, self.W) >= 0.1 * self.cfg["sc_factor"]
            ) & (mask)
            pts = xyz_map[valid].reshape(-1, 3)
            pts = transform_pts(pts, cvcam_in_ob)
            ...
            common.rayColorToTextureImageCUDA(
                torch.from_numpy(mesh.faces).cuda().long(),
                torch.from_numpy(mesh.vertices).cuda().float(),
            ...
            uvs_flat_unique, inverse_ids, cnts = torch.unique(
                uvs_flat, return_counts=True, return_inverse=True
            )
            perm = torch.arange(inverse_ids.size(0)).cuda()
            inverse_ids, perm = inverse_ids.flip([0]), perm.flip([0])
            unique_ids = inverse_ids.new_empty(uvs_flat_unique.size(0)).scatter_(
                0, inverse_ids, perm
            )
            uvs_unique = torch.stack(
                (uvs_flat_unique % (W - 1), uvs_flat_unique // (W - 1)), dim=-1
            ).reshape(-1, 2)
            cur_weights = torch.ones((len(uvs_unique))).cuda().float()
            tex_image[uvs_unique[:, 1], uvs_unique[:, 0]] += torch.from_numpy(
                ray_colors
            ).cuda().float()[unique_ids] * cur_weights.reshape(-1, 1)
        ...
        mesh.visual = trimesh.visual.texture.TextureVisuals(
            uv=mesh.visual.uv, image=Image.fromarray(tex_image)
        )
        return mesh
