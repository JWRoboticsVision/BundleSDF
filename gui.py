...
class BundleSdfGui:
    def __init__(self, img_height=300):
        ...
        with dpg.window(label="", tag="main"):
            with dpg.group(horizontal=True, tag="buttons"):
                dpg.add_button(
                    label="clean_mesh", tag="clean_mesh", callback=self.clean_mesh
                )
                dpg.add_file_dialog(
                    directory_selector=True,
                    show=False,
                    callback=self.export_mesh,
                    tag="export_mesh_file_dialog",
                    label="export mesh",
                    default_filename="1.obj",
                    height=600,
                    width=1000,
                )
                dpg.add_button(
                    label="export_mesh",
                    tag="export_mesh",
                    callback=lambda: dpg.show_item("export_mesh_file_dialog"),
                )
            ...
            with dpg.handler_registry():
                dpg.add_mouse_drag_handler(
                    button=dpg.mvMouseButton_Left, callback=self.drag_rotate_pose
                )
                dpg.add_mouse_drag_handler(
                    button=dpg.mvMouseButton_Right, callback=self.drag_move_pose
                )
        ...
        dpg.add_text(
            "keyframe_num: 0", tag="keyframe_num", color=[0, 255, 0], parent="row3"
        )
        dpg.add_text(
            "nerf_num_frames: X",
            tag="nerf_num_frames",
    ...
    def update_frame(self, rgb, mask, ob_in_cam, id_str, K, n_keyframe):
        ...
        if dpg.get_value("rgb") is None:
            with dpg.texture_registry(show=False):
                dpg.add_dynamic_texture(
                    self.W, self.H, rgba.reshape(-1) / 255.0, tag="rgb_init"
                )
                dpg.add_dynamic_texture(
                    self.W,
                    self.H,
                    masked_rgba.reshape(-1) / 255.0,
                    tag="masked_rgb_init",
                )
                dpg.add_dynamic_texture(
                    self.W, self.H, rgba.reshape(-1) / 255.0, tag="rgb"
                )
                dpg.add_dynamic_texture(
                    self.W, self.H, masked_rgba.reshape(-1) / 255.0, tag="masked_rgb"
                )
        ...
        if dpg.get_value("mesh_render") is None:
            with dpg.texture_registry(show=False):
                dpg.add_dynamic_texture(
                    self.W, self.H, np.zeros((4), dtype=np.float32), tag=f"mesh_render"
                )
    ...
    def update_mesh(self, mesh):
        ...
        if self.renderer is None:
            ...
            self.renderer = TinyRenderer(
                H=self.H, W=self.W, K=self.K
            )  #!NOTE other renderer needs openGL context and not work
